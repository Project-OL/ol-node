...
export const giftGalleryService = {
  async upsertGallery(params: {
    ...
    try {
      await redisClient.del(
        RedisKeys.giftGalleryTemplate(params.year, params.month),
      );
    } catch {
      // ignore
    }
  ...
  async getGalleryForHost(hostUserId: string): Promise<GalleryResponse> {
    ...
    const progressRows = await giftGalleryRepository.findHostProgress(
      gallery.id,
      hostUserId,
    );
  ...
  async getCompletionSummaryForUser(
    hostUserId: string,
  ): Promise<GalleryCompletionSummary> {
    const { year, month, secondsRemaining } = getActivePeriod();
    const monthEndAt = getMonthEndIso();
    const hostKey = RedisKeys.giftGalleryHost(hostUserId, year, month);
    try {
    ...
    const s = await giftGalleryRepository.getHostCompletionSummary(
      gallery.id,
      hostUserId,
    );
    return {
  ...
  async recordGiftProgress(params: {
    ...
    if (created) {
      try {
        await redisClient.del(
          RedisKeys.giftGalleryHost(params.hostUserId, year, month),
        );
      } catch {
        // ignore
      }
  ...
  async invalidateHostMonthCache(
    hostUserId: string,
    year: number,
    month: number,
  ): Promise<void> {
