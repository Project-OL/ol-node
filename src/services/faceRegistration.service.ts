...
export const faceRegistrationService = {
  ...
  async createSession(
    ...
    if (!awsSessionId) {
      throw new AppError(
        502,
        'Face liveness session could not be created',
        'FACE_REG_LIVENESS_CREATE_FAILED',
      )
    }
  ...
  async requestVerify(
    ...
    if (env.FACE_REGISTRATION_SUPPLEMENTAL_VIDEO_REQUIRED) {
      if (!session.supplementalVideoS3Key?.trim()) {
        throw new AppError(
          400,
          'Supplemental video upload is required before verify',
          'FACE_REG_VIDEO_REQUIRED',
        )
      }
    }
    ...
    if (!locked) {
      throw new AppError(
        409,
        'Another face registration operation is in progress',
        'FACE_REG_LOCK_BUSY',
      )
    }
    ...
    try {
      ...
      if (session.status !== 'PENDING' && session.status !== 'UPLOADED') {
        throw new AppError(
          409,
          'Session cannot be verified from this state',
