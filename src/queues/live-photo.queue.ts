...
export async function enqueueLivePhotoVerification(data: LivePhotoVerifyJobData): Promise<void> {
...
export async function enqueueLivePhotoS3Purge(keys: string[]): Promise<void> {
  ...
  const purgeKey = createHash('sha256').update(keys.join('\0')).digest('hex').slice(0, 16)
