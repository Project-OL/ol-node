...
const ALL_CATEGORY_TX_TYPES = new Set<PointTxType>(
  Object.values(POINT_EARNINGS_CATEGORIES).flat(),
);
...
export function isPointEarningsCategory(
  value: string,
): value is PointEarningsCategory {
 ...
 */
export function resolvePointHistoryTxTypes(
  filters?: string[],
): PointTxType[] | undefined {
...
export function categoryIncludesTxType(
  ...
  return (POINT_EARNINGS_CATEGORIES[category] as readonly PointTxType[]).includes(
    txType,
  );
}

/** Summary/history earnings bucket for a ledger tx type, if any. */
export function earningsCategoryForTxType(
  txType: PointTxType,
): PointEarningsCategory | null {
  for (const key of POINT_EARNINGS_CATEGORY_KEYS) {
