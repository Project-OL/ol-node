...
const dashboardPeriodSchema = z
  .object({
    period: z.enum(['TODAY', 'YESTERDAY', 'THIS_WEEK', 'THIS_MONTH', 'LAST_30_DAYS']).optional(),
    from: z
      .string()
      .regex(/^\d{4}-\d{2}-\d{2}$/)
      .optional(),
 ...
 */
export async function registerAgencyDashboardRoutes(app: FastifyInstance): Promise<void> {
  app.get(
    '/dashboard/earnings',
