...
import { rateLimitAgencyApply, rateLimitAgencyLeaveApply } from '../../middlewares/rateLimitAuth'
import { AppError } from '../../middlewares/errorHandler'
import { agencyService, mapHostAgencyMeBlock } from '../../services/agency.service'
...
import { rateLimitPayrollComplete, rateLimitPayrollReject } from '../../middlewares/rateLimitAuth'
...
export default async function agencyRoutes(app: FastifyInstance) {
  ...
  app.get('/me', { preHandler: preAuth }, async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const pendingLeave = await agencyLeaveApplicationRepository.getPendingForHost(userId)
    return reply.send({
      owned: view.owned
        ? {
  ...
  app.delete<{ Params: { id: string } }>(
    ...
    async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
      ...
      if (app.status === 'ACCEPTED') {
        throw new AppError(409, 'Cannot cancel an accepted application', 'INVALID_STATE')
      }
  ...
  app.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (!userId) throw new AppError(401, 'Unauthorized', 'UNAUTHORIZED')
      const q = request.query as Record<string, string | undefined>
      const status = q.status as 'PENDING' | 'ACCEPTED' | 'REJECTED' | 'CANCELLED' | undefined
  ...
  const cancelLeaveApplicationHandler = async (
    ...
    const userId = request.userId
    if (!userId) throw new AppError(401, 'Unauthorized', 'UNAUTHORIZED')
    const result = await agencyHostService.cancelLeaveApplication(userId, request.params.id)
    return reply.send(result)
  }
  ...
  app.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const cursor = q.cursor ?? undefined
      const result = await agencyHostService.listLeaveApplicationsInbox(agencyRow.userId, {
        limit,
        cursor,
      })
      return reply.send(result)
  ...
  app.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      return reply.send(
        await withdrawalService.getAgentPayrollInboxPending(userId, query.cursor, query.limit),
      )
    },
  )
  ...
  app.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const query = PayrollInboxTabQuerySchema.parse(request.query ?? {})
      return reply.send(
        await withdrawalService.getAgentPayrollInboxWaiting(userId, query.cursor, query.limit),
      )
    },
  )
  ...
  app.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const query = PayrollInboxTabQuerySchema.parse(request.query ?? {})
      return reply.send(
        await withdrawalService.getAgentPayrollInboxSettled(userId, query.cursor, query.limit),
      )
    },
  )
  ...
  app.get<{ Params: { id: string } }>(
    ...
    async (request, reply: FastifyReply) => {
      const userId = request.userId!
      const detail = await agencyService.getAssignmentDetail(request.params.id, userId)
      if (detail.status === 'PENDING' && detail.slaRemainingSeconds > 0) {
        auditService.log({
          userId,
  ...
  app.post<{ Params: { id: string } }>(
    ...
    async (request, reply: FastifyReply) => {
      ...
      const result = await withdrawalService.agentCompletePayroll(userId, request.params.id, body)
  ...
  app.post<{ Params: { id: string } }>(
    ...
    async (request, reply: FastifyReply) => {
      ...
      await withdrawalService.agentRejectPayroll(userId, request.params.id, body.reason)
      return reply.send({ ok: true })
    },
  )
